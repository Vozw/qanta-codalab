...
def load_data(filename):
    ...
    with open('/Users/riyachanduka/Documents/GitHub/qanta-codalab/data/sorted_answer_dict.json', encoding = 'utf-8') as data_file:
        sorted_answer = json.loads(data_file.read())
    sorted_answer_list = list(sorted_answer)
    ...
    with open(filename) as json_data:
        ...
        for q in questions:
            ...
            label = q['label']
            if label in sorted_answer_list:
                label = sorted_answer_list.index(label)
    ...
    print('My length is', len(list(set(label_list))))
    return data
...
def vectorize(ex, word2ind):
    ...
    for index,text in enumerate(question_text):
        ...
        if text in word2ind.keys():
            vec_text[index]=word2ind[text]
            
        else:
            vec_text[index]=word2ind['<unk>']
...
def train(args, model, train_data_loader, dev_data_loader, accuracy, device):
    ...
    for idx, batch in enumerate(train_data_loader):
        ...
        optimizer.zero_grad()
        #out = model(batch)
        prediction = model(question_text, question_len)
...
class DanModel(nn.Module):
    ...
    def __init__(self, n_classes, vocab_size, emb_dim=50,
        ...
        self.linear2 = nn.Linear(n_hidden_units, n_classes+1)
    ...
    def forward(self, input_text, text_len, is_prob=False):
        ...
        input_embed = self.embeddings(input_text)

        encoded=input_embed.sum(1)
        encoded /= text_len.view(input_embed.size(0),-1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train-file', type=str, default='/Users/riyachanduka/Documents/GitHub/qanta-codalab/data/train_data_DAN.json')
    parser.add_argument('--dev-file', type=str, default='/Users/riyachanduka/Documents/GitHub/qanta-codalab/data/dev_data_DAN.json')
    parser.add_argument('--test-file', type=str, default='/Users/riyachanduka/Documents/GitHub/qanta-codalab/data/test_data_DAN.json')
